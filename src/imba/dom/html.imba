...
extend tag input
	...
	def value
		@dom.type == 'number' ? parseFloat(@dom.value or 0) : @dom.value

	def oninput e
		let val = @dom.value
		@localValue = val
		if @data and !@lazy and @dom.type != 'radio' and @dom.type != 'checkbox'
			@data.setFormValue(@value(),self) # TODO value is function
		return
...
extend tag select
	...
	def syncValue value
		let prev = #syncedValue
		# check if value has changed
		if @dom.multiple and value isa Array
		...
		if typeof value == 'object'
			let mult = @dom.multiple and value isa Array
