...
CLI.prototype.findRecursive = function (root,pattern){
	if(pattern === undefined) pattern = /\.imba1?$/;
	var results = [];
...
CLI.prototype.pathToSource = function (src,coll,o,root){
	...
	if (file.targetPath) {
		file.targetPath = file.targetPath.replace(/\.imba1?$/,'.js');
	};
...
CLI.prototype.analyze = function (){
	...
	return self.traverse(function(src) {
		...
		src.analysis = out;
		(((v_ = out.tokens),delete out.tokens, v_));
...
CLI.prototype.migrate = function (){
	...
	return self.traverse(function(src) {
		...
		for (var i = 0, items = iter$(out.tokens), len = items.length, token; i < len; i++) {
			token = items[i];
			if (token._type == '.:' && token._loc) {
				edits.push([token._loc,'.',1]);
			};
		};
		
		for (var j = 0, ary = iter$(out.autoself), len_ = ary.length; j < len_; j++) {
			edits.push([ary[j].loc[0],'self.',0]);
		};
		
		for (var i_ = 0, array = iter$(out.autocall), len__ = array.length; i_ < len__; i_++) {
			edits.push([array[i_].loc[1],'()',0]);
		};
		...
		self.present(result);
		return fs.writeFileSync(targetPath,result,'utf8');
...
CLI.prototype.finish = function (){
	try {
		...
		} else if (this.o().migrate) {
			this.o().print = true;
			this.migrate(this.sources(),this.o());
		} else {
