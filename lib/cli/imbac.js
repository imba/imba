...
function ensureDir(src){
	...
	for (var i = 0, items = iter$(parts), len = items.length; i < len; i++) {
		if (i < 1) { continue; };
...
function findRecursive(root,pattern){
	...
	var read = function(src,depth) {
		...
		if (stat.isDirectory() && depth > 0) {
			...
			for (var i = 0, items = iter$(files), len = items.length, res = []; i < len; i++) {
...
function pathToSource(src,coll,o,root){
	...
	if (stat.isDirectory()) {
		...
		for (var i = 0, items = iter$(files), len = items.length; i < len; i++) {
			pathToSource(items[i],coll,o,abs);
		};
		return;
	};
...
CLI.prototype.run = function (){
	...
	for (var i = 0, items = iter$(paths), len = items.length; i < len; i++) {
		pathToSource(items[i],self._sources,self._options);
	};
...
CLI.prototype.tokenize = function (){
	...
	return self.traverse(function(src) {
		...
		for (var strings = [], i = 0, items = iter$(src.tokens), len = items.length, t; i < len; i++) {
			t = items[i];
			var typ = t._type;
			var id = t._value;
			var s;
