(function(){
	...
	Imba.extendTag('htmlelement', function(tag){
		...
		tag.prototype.setStaticChildren = function (nodes){
			...
			if (prev == null) {
				tag.__super__.setStaticChildren.apply(this,arguments);
		...
		tag.prototype.removeNode = function (node,tail){
		...
		tag.prototype.skipNode = function (node,tail){
			if (node instanceof Array) {
				for (var i=0, ary=iter$(node), len=ary.length; i < len; i++) {
		...
		tag.prototype.insertNode = function (node,tail){
			if (node instanceof Array) {
				for (var i=0, ary=iter$(node), len=ary.length; i < len; i++) {
				...
				var domNode = Imba.document().createTextNode(this.item());
		...
		tag.prototype.reconcileLoop = function (prevNodes,nodes,tail){
			var removedNodes = 0;
			var isSorted = true;
		...
		tag.prototype.reconcileChanges = function (prevNodes,nodes,tail){
			// TODO
			return this.reconcileScratch(prevNodes,nodes,tail);
		};
		
		tag.prototype.reconcileSwap = function (nodes,groups){
		...
		tag.prototype.reconcileOrder = function (nodes,groups){
			var last;
		...
		tag.prototype.moveGroupBeforeTail = function (nodes,group,tail){
		...
		tag.prototype.moveGroup = function (nodes,group,nextGroup){
			var tail = nodes[nextGroup[0]]._dom.nextSibling;
			return this.moveGroupBeforeTail(nodes,group,tail);
		};
		
		tag.prototype.swapGroup = function (nodes,group1,group2){
			var group,tail;
		...
		tag.prototype.reconcileScratch = function (prevNodes,nodes,tail){
			for (var i=0, ary=iter$(prevNodes), len=ary.length; i < len; i++) {
				tail = this.removeNode(ary[i],tail);
			};
