...
function escapeTextContent(val,nodeName){
	var str = (typeof val == 'string') ? val : String(val);
...
ImbaServerElement.prototype.appendNested = function (child){
	if (child instanceof Array) {
		...
		this.appendChild(child._dom || child);
	};
	return;
};
...
ImbaServerElement.prototype.setAttribute = function (key,value){
	...
	this._attributes.push(("" + key + "=\"" + escapeAttributeValue(value) + "\""));
...
ImbaServerElement.prototype.resolve = function (){
	if (this._resolvedChildren != this._tag._children) {
		var content = this._tag._children;
