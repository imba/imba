...
Object.defineProperty(Imba.Tag.prototype,'root',{get: function(){
	return this.owner_ ? this.owner_.root : this;
}, configurable: true});
...
Object.defineProperty(Imba.Tag.prototype,'text',{set: function(txt){
	// TODO fix get/set combined into a single thing
	this.__tree_ = txt;
	this.dom.textContent = (txt == null || this.text === false) ? '' : txt;
	return this;
}, configurable: true});
...
Imba.Tag.prototype.flagIf = function (flag,bool){
	var f = this.__flags_ || (this.__flags_ = {});
	let prev = f[flag];
...
Imba.createElement = function (name,ctx,ref,pref){
	...
	if (ctx instanceof TagMap) {
		...
		parent = (ctx && pref != undefined) ? ctx[pref] : ((ctx && ctx.tag || ctx));
	};
