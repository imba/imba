...
function Imba2TagProxy(node){
	this._target = node;
	this._cache = {};
	return this;
};

Imba2TagProxy.build = function (node){
	return new Proxy(node,new this(node));
};

Imba2TagProxy.prototype.setter = function (key,receiver){
	var self = this;
	return function(value,mods) {
		// console.log 'setting!!',key,value,@target
		self._target[key] = value;
		return receiver;
	};
};

Imba2TagProxy.prototype.get = function (target,key,receiver){
	var cache = (target.V1CACHE || (target.V1CACHE = {}));
	...
	if (key == 'dom') {
		return cache[key] = function() { return target; };
	};
	if (key == 'tag') {
		return cache[key] = function() { return target; };
	};
	
	if (key == '_tag' || key == '_dom' || key == '_slot_') {
		return target;
	};
	...
	if (key.match(/^set[A-Z]/)) {
		var name = key[3].toLowerCase() + key.slice(4);
		return cache[key] = this.setter(name,receiver);
	} else {
...
Imba.SVGTag.inherit = function (child){
	...
	if (this == Imba.SVGTag) {
		...
		if (Imba.TAG_AUTOCLASS_SVG && child._flagName) {
			classes.push(child._flagName);
		};
...
Imba.Tags.prototype.defineTag = function (o,fullName,supr,body){
	...
	if ((o && o.scope && o.scope.flags())) {
		(_classes_ = tagtype._classes).push.apply(_classes_,o.scope.flags());
	};
...
Imba.createElement = function (name,ctx,ref,pref){
	...
	if (type == null && globalThis && globalThis.imba) {
		node = globalThis.imba.document.createElement(name);
		node = Imba2TagProxy.build(node);
		// node['init$']() if node['init$']
	} else {
		node = type.build(parent);
	};
...
TagScope.prototype.ns = function(v){ return this._ns; }
TagScope.prototype.setNs = function(v){ this._ns = v; return this; };
TagScope.prototype.flags = function(v){ return this._flags; }
TagScope.prototype.setFlags = function(v){ this._flags = v; return this; };
