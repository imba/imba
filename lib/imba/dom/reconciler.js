...
function reconcileCollectionChanges(root,new$,old,caret){
	...
	for (var idx1 = 0, ary = iter$(new$), len_ = ary.length, node1; idx1 < len_; idx1++) {
		...
		if (!stickyNodes[idx1]) {
			...
			insertNestedAfter(root,node1,(after && after._slot_ || after || caret));
		};
...
Imba.extendTag('element', function(tag){
	...
	tag.prototype.setChildren = function (new$,typ){
		...
		if (!old && typ != 3) {
			...
			this.__root().appendNested(this,new$);
		} else if (typ == 1) {
			var caret = null;
			for (var i = 0, items = iter$(new$), len = items.length; i < len; i++) {
				caret = this.__root().reconcileNested(this,items[i],old[i],caret);
			};
		} else if (typ == 2) {
			return this;
		} else if (typ == 3) {
			...
			if (new$ && new$._dom) {
				...
				if (new$._type == 5 && old && old._type == 5) {
					this.__root().reconcileLoop(this,new$,old,null);
				} else if (old instanceof Array) {
					this.__root().reconcileNested(this,new$,old,null);
				} else {
					this.removeAllChildren();
					this.__root().appendNested(this,new$);
				};
			...
			this.__root().reconcileIndexedArray(this,new$,old,null);
		} else if (typ == 5) {
			this.__root().reconcileLoop(this,new$,old,null);
