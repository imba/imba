(function(){
	...
	function reconcileChanges(root,new$,old,caret){
		return caret;
	};
	...
	function reconcileCollection(root,new$,old,caret){
		...
		for (var i=0, ary=iter$(old), len=ary.length; i < len; i++) {
			var newIdx = new$.indexOf(ary[i]);
		...
		if (isSorted) {
			if (hasChanges) {
				return reconcileChanges(old,new$,this.tail());
			} else {
				return this.tail();
			};
		} else {
			if (!hasChanges && groups.length == 2) {
				return this.reconcileSwap(new$,groups);
			} else if (!hasChanges && groups.length == 3) {
				return this.reconcileOrder(new$,groups);
			} else {
				return this.reconcileScratch(old,new$,this.tail());
			};
		};
	};
