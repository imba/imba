	...
	var grammar = {
		...
		Block: [
			o('INDENT OUTDENT',function (){
				return new Block([]).indented(A1,A2).set({ends: [A1,A2]});
			}),
			o('INDENT Body OUTDENT',function (){
				return A2.indented(A1,A3).set({ends: [A1,A3]});
			}),
			// hacky way to support terminators at the start of blocks
			o('INDENT TERMINATOR Body OUTDENT',function (){
				return A3.prebreak(A2).indented(A1,A4).set({ends: [A1,A4]});
			})// rather indent, no?
		],
		...
		Property: [
			o('PROP PropertyIdentifier Object',function (){
				return new PropertyDeclaration(A2,A3,A1);
			}),
			o('PROP PropertyIdentifier CALL_START Object CALL_END',function (_0){
				return new PropertyDeclaration(A2,A4,_0);
			}),
		...
		VarReference: [
			o('VAR SPLAT VarIdentifier',function (){
		...
		AssignList: [
			...
			o('AssignList OptComma INDENT AssignList OptComma Outdent',function (){
				return A1.concat(A4.indented(A3,A6));// hmmm
			})
		],
		...
		Operation: [
			...
			o('-- SimpleAssignable',function (){
				return AST.OP('--',null,A2);
			}),
			o('++ SimpleAssignable',function (){
				return AST.OP('++',null,A2);
			}),
			o('SimpleAssignable --',function (){
				return AST.OP('--',A1,null,true);
			}),
			o('SimpleAssignable ++',function (){
				return AST.OP('++',A1,null,true);
			}),
	...
	for(var name in o1){
		for(var i=0, ary=iter$(o1[name]), len=ary.length, alt, res=[]; i < len; i++) {
			alt = ary[i];for(var j=0, items=iter$(alt[0].split(' ')), len_=items.length, token; j < len_; j++) {
				token = items[j];if(!(grammar[token])) {
					tokens.push(token);
				};
			};
			if(name == 'Root') {
