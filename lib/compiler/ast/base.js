...
AST.Value = imba$class(function Value(value){
	(this.setValue(value), value);
},AST.Node);

AST.Value.prototype.__value = {};
AST.Value.prototype.value = function(v){ return this._value; }
AST.Value.prototype.setValue = function(v){ this._value = v; return this; }
;

AST.Value.prototype.visit = function (){
	return (this.value() && this.value().traverse) && (this.value().traverse());
};
AST.Value.prototype.js = function (){
	return this.value().c();
};
...
AST.ListNode.prototype.unshift = function (item){
...
AST.Comment.prototype.c = function (){
	return "/* " + (this.value().c()) + " */";
};
...
AST.Block.prototype.push = function (item,sep){
	if(sep) {
		var ln = sep.split("\\n").length;
		this.last() && (this.last()._newlines = ln - 1);
	};
	this.nodes().push(item);
	return this;
};
...
AST.Block.prototype.js = function (o){
	...
	return (this.isExpression() || o.isExpression() || (this.option('express') && this.isExpressable())) ? (ast.c().flatten().compact().join(", ")) : (ast = ast.map(function (node,i){
		var c = node.c();
		if(c == "") {
			return null;
		};
		...
		var ln = node._newlines || 1;
		return c += Array(ln + 1).join("\n");
	}), ast.compact().filter(filter).join("").replace(/[\s\n]+$/,''));
};
AST.Block.prototype.replace = function (original,replacement){
	var idx = this.nodes().indexOf(original);
	if(idx >= 0) {
		this.nodes()[idx] = replacement;
	};
...
AST.Block.prototype.last = function (){
	var i = this.nodes().length;
	while(i){
		i = i - 1;
		var v = this.nodes()[i];
		if(!((v instanceof AST.Comment))) {
			return v;
		};
	};
	return null;
