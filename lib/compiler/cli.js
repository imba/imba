(function(){
	...
	SourceFile.prototype.name = function (){
		return this._path.split("/").pop(); // for testing
	};
	...
	SourceFile.prototype.analyze = function (o,cb){
		...
		try {
			this._meta = this.ast().analyze({loglevel: 0,entities: o.entities,scopes: true,target: o.target});
			cb && cb(this._meta);
		} catch (e) {
	...
	SourceFile.prototype.run = function (){
		process.argv.shift();
		process.argv[0] = 'imba';
		return Imbac.run(this.code(),{filename: this._path,target: 'node'});
	};
	
	
	SourceFile.prototype.compile = function (o){
		if(o === undefined) o = {};
		o.filename = this._path;
		o.target || (o.target = 'node');
		
		var res = {};
		try {
			return res = Imbac.compile(this.code(),o);
		} catch (e) {
	...
	function printTokens(tokens){
		for (var strings = [], i = 0, ary = iter$(tokens), len = ary.length, t; i < len; i++) {
			...
			var s;
			if (typ == 'TERMINATOR') {
				strings.push(("(" + (t._loc) + ":" + (t._len) + ")[") + chalk.yellow(id.replace(/\n/g,"\\n")) + "]\n");continue;
			};
	...
	}; exports.sourcefileForPath = sourcefileForPath;
	...
	function compileFile(source,opts){
	...
	cli.command('compile <path>').description('compile scripts').option('-m, --source-map-inline','Embed inline sourcemap in compiled JavaScript').option('-s, --standalone','Embed utils from Imba runtime').option('-t, --target [platform]','Compile for specific platform').option('-b, --bare','Skip wrapping code in outer closure').option('-o, --output [dest]','set the output directory for compiled JavaScript').action(function(path,o) { return cliCompile(path,o,{watch: false}); });
	
	
	cli.command('find <path>').description('compile scripts').action(function(src,o) {
		log(("action with path " + src));
		return log("files:" + findFiles(src).join("\n"));
	});
	
	
	cli.command('watch <path>').description('listen for changes and compile scripts').option('-m, --source-map-inline','Embed inline sourcemap in compiled JavaScript').option('-s, --standalone','Embed utils from Imba runtime').option('-t, --target [platform]','Compile for specific platform').option('-b, --bare','Skip wrapping code in outer closure').option('-o, --output [dest]','set the output directory for compiled JavaScript').action(function(path,o) { return cliCompile(path,o,{watch: true}); });
	
	
	cli.command('analyze <path>').description('get information about scopes, variables and more').option('--tokens','print the raw tokens').option('--target [platform]','Compile for specific platform').option('-e, --entities','print the raw tokens').action(function(path,opts) {
		var file = sourcefileForPath(path);
