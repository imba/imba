(function(){
	...
	function writeFile(source,outpath){
		...
		try {
			...
			if (ok) { fs.writeFileSync(outpath,code.js || code) };
		} catch (e) {
	...
	cli.command('compile <path>').description('compile scripts').option('-o, --output [dest]','set the output directory for compiled JavaScript').action(function(path,o) { return cliCompile(path,o,{watch: false}); });
	
	cli.command('watch <path>').description('listen for changes and compile scripts').option('-o, --output [dest]','set the output directory for compiled JavaScript').option('--cache-tag-attributes','set the output directory for compiled JavaScript').action(function(root,o) {
