	...
	Lexer.prototype.interpolateString = function (str,options){
		...
		while (letter = str.charAt(i += 1)){
			...
			if (pi < i) {
				...
				var tok = new Token('NEOSTRING',this.escapeStr(str.slice(pi,i),heredoc,quote),this._line,this._loc + pi + locOffset,i - pi);
				// tok.@loc = @loc + pi
				// tok.@len = i - pi + 2
				tokens.push(tok);
			};
		...
		if (i >= pi && pi < str.length) {
			...
			tokens.push(new Token('NEOSTRING',this.escapeStr(str.slice(pi),heredoc,quote),0,this._loc + pi + locOffset,str.length - pi));
		};
