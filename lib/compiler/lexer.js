...
// Changed so it's aware of '!' symbol
var TAG = /^(\<|%|\<\!|%\!)(?=[A-Za-z\#\.\{\@\>])/;
...
Lexer.prototype.tagToken = function (){
	...
	var ary = iter$(match);var input = ary[0],type = ary[1][0],identifier = ary[2];
	// Get the '!' symbol to make our lexer aware of it
	var slotSymbol = ary[1][1];
	if (type == '<') {
		...
		if (match = TAG_TYPE.exec(this._chunk.substr(1,40))) {
			...
			// Check if it has that symbol and create that token
			if (slotSymbol === '!'){
				this.token('TAG_SLOT',match[2],match[2].length,1);
				return input.length + match[2].length;
			}
			// If '!' is emitted and it's not self tag then do this
			if (match[0][0] !== '!' && match[0] != 'self') {
