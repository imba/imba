(function(){
	...
	Lexer.prototype.pushEnd = function (val,ctx){
		...
		if (ctx && ctx.id) {
			ctx.open = new Token(ctx.id + '_OPEN',val,this._last.region()[1],0);
			this._tokens.push(ctx.open);
		};
		return this;
	};
	...
	Lexer.prototype.popEnd = function (val){
		...
		if (ctx && ctx.open) {
			ctx.close = new Token(ctx.id + '_CLOSE',popped,this._last.region()[1],0);
			ctx.close._opener = ctx.open;
			ctx.open._closer = ctx.close;
			this._tokens.push(ctx.close);
		};
	...
	Lexer.prototype.tagContextToken = function (){
		...
		if (match = TAG_ATTR.exec(this._chunk)) {
			...
			this._loc += l - 1;
			this.token('=','=',1);
			this.pushEnd('TAG_ATTR',{id: 'TAG_ATTR_VALUE',pop: /^[\s\n\>]/}); //  [' ','\n','>']
			return l;
		};
		return 0;
	};
