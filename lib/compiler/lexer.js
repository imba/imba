(function(){
	...
	Lexer.prototype.literalToken = function (){
		...
		if(value == ';') {
		...
		} else if(value == '*' && this._chunk.charAt(1).match(/[A-Za-z\_\@\[]/) && (prev.spaced || idx$(prev[1],[',','(','[','{','|','\n','\t']) >= 0)) {
			tokid = "SPLAT";
		} else if(value == '√') {
			tokid = 'SQRT';
		} else if(value == 'ƒ') {
			...
			} else if(value == '(' && idx$(prev[0],CALLABLE) >= 0) {
				// not using this ???
				// prev[0] = 'FUNC_EXIST' if prev[0] is '?'
				tokid = 'CALL_START';
			} else if(value == '[' && idx$(prev[0],INDEXABLE) >= 0) {
				tokid = 'INDEX_START';
				if(prev[0] == '?') {
					prev[0] = 'INDEX_SOAK';
				};
	...
	var IDENTIFIER = /^((\$|@@|@|\#)[A-Za-z_\-\x7f-\uffff][$\w\x7f-\uffff]*(\-[$\w\x7f-\uffff]+)*|[$A-Za-z_][$\w\x7f-\uffff]*(\-[$\w\x7f-\uffff]+)*)([^\n\S]*:(?![\*\=:$\w\x7f-\uffff]))?/;
