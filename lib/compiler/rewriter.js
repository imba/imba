(function(){
	...
	Rewriter.prototype.detectEnd = function (i,condition,action,state){
		...
		while (i < tokens.length){
			...
			if (!token || levels < 0) {
				return action.call(this,token,i - 1,tokens);
			};
	...
	Rewriter.prototype.closeOpenCalls = function (){
		var condition = function(token,i,starts,tokens) {
			var t = token._type;
	...
	Rewriter.prototype.addImplicitIndentation = function (){
		...
		var indentAction = function(token,i,tokens) {
			var idx = this.tokenType(i - 1) === ',' ? ((i - 1)) : (i);
			tokens.splice(idx,0,T.OUTDENT);
			return;
		};
		
		var indentCondition = function(token,i,starts,tokens) {
			var t = token._type;
		...
		while (i < tokens.length){
			...
			if (SINGLE_LINERS[type] && (next != INDENT && next != 'BLOCK_PARAM_START') && !(type == 'ELSE' && next == 'IF') && type != 'ELIF') {
				...
				this.detectEnd(i + 2,indentCondition,indentAction);
	...
	Rewriter.prototype.tagPostfixConditionals = function (){
		var condition = function(token,i,starts,tokens) { return token._type === TERMINATOR || token._type === INDENT; };
