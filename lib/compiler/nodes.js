...
For.prototype.declare = function (){
	...
	if (src instanceof Range) {
		...
		if (oi) {
			...
			vars.index = scope.declare('i',new Num(0),{system: true,type: 'counter',unique: true});
		};
		
		vars.source = bare ? src : scope.declare('ary',this.util().iterable(src),{system: true,type: 'iter',unique: true});
		vars.len = scope.declare('len',this.util().len(vars.source),{system: true,type: 'len',unique: true});
