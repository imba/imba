...
Tag.prototype.js = function (o){
	...
	var ctor = [
		typ,
		(this._parent ? this._parent.ref() : 'null'),
	...
	if (!this._parent) {
		...
		if (this.isSelf()) {
			...
			add(("var " + vars));
			add("b$ = 1");
			add(("t$ = this." + this.oid() + "$$ || (b$=0,this." + this.oid() + "$$ = " + this.create_() + "(" + ctor.join(',') + "))"));
		...
		add(("t$ = " + this.ref() + " || (b$=0," + this.ref() + " = " + this.create_() + "(" + ctor.join(',') + "))"));
		this._ref = "t$";
		add(("c$=t$.$ || (t$.$=\{\})"));
	} else {
		...
		if (canInline) {
			...
			for (let i = 0, len = statics.length; i < len; i++) {
				res.push(("el$." + statics[i].c(o)));
			};
			var parts = res;
			if (parts.length > 0) { parts.unshift('') };
			return ("b$ || (el$=" + this.create_() + "(" + ctor.join(',') + ")" + parts.join(",") + ")");
		} else {
			...
			add(("" + this.ref() + " || (" + (markWhenBuilt ? 'bel$=0,' : '') + this.ref() + " = " + this.create_() + "(" + ctor.join(',') + "))"));
		};
	};
