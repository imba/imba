...
MethodDeclaration.prototype.visit = function (){
	...
	if (String(this.name()) == 'initialize') {
		if ((closure instanceof ClassScope) && !(closure instanceof TagScope)) { // and not ModuleScope?
	...
	if ((closure instanceof ClassScope) && !(this.target())) {
		this._target = closure.prototype(); // ||= ValueNode.new(OP('.',closure.context,'prototype'))
	...
	if (this.target() instanceof Self) {
		...
		if (closure instanceof ModuleScope) {
			...
			this._target = closure.context().reference();
...
Export.prototype.loc = function (){
	...
	return (kw && kw.region) ? kw.region() : (Export.__super__.loc.apply(this,arguments));
};
...
Export.prototype.visit = function (){
	...
	return Export.__super__.visit.apply(this,arguments);
};
...
ModuleScope.prototype.isClosed = function (){
	return true;
};
