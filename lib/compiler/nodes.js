...
MethodDeclaration.prototype.visit = function (){
	...
	if (!this._target) {
		if (o.variable) {
			...
			this._variable = this.context().register(this.name(),this,{type: String(o.variable)});
		} else if (this.context() instanceof RootScope) {
	...
	if (variable && variable.declarator()) {
		...
		if (variable && variable._initialized || (scope.closure() != vscope.closure())) {
			...
			if ((vscope instanceof RootScope) && vscope.context() != scope.context() && variable.type() == 'meth') {
				this.warn(("calling method from root scope " + this.value() + " is deprecated - see issue #112"));
			};
...
RootScopeContext.prototype.reference = function (){
	// should be a 
	return this._reference || (this._reference = this.scope().declare("$root$",this.scope().object(),{type: 'global'}));
};
