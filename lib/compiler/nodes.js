...
// Added this to make sure it's different with normal TAG
function TagSlot(token){
	this._token = token;
	this._value = this.load(token);
	return this;
}

subclass$(TagSlot, TagTypeIdentifier);
exports.TagSlot = TagSlot;
...
Tag.prototype.isSlot = function(){
	return (this.type() instanceof TagSlot);
}
...
Tag.prototype.js = function (jso){
	...
	var content = o.body;
	// Added check if isSlot is true then we set it to "setSlot"
	var bodySetter = this.isSelf() ? "setChildren" : this.isSlot() ? "setSlot" : "setContent";
