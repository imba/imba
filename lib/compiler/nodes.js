...
Tag.prototype.js = function (o){
	...
	var ctor = [typ,(this._parent ? this._parent.ref() : 'null'),(this._slot !== null) ? this._slot : 'null'];
	var commit = false;
	...
	if (this._level == 1) {
		...
		add(("" + this.ref() + " || (" + this.ref() + " = " + this.create_() + "(" + ctor.join(',') + "))"));
	};
	...
	var slot = 0;
	for (var k = 0, array = iter$(nodes), len__ = array.length, item2; k < len__; k++) {
		item2 = array[k];
		if (item2 instanceof Tag) {
			item2._slot = slot++;
			add(item2.c(o));
		} else if ((item2 instanceof Str) && false) {
			add(("" + this.ref() + ".render_(" + item2.c(o) + "," + (slot++) + ")"));
		} else {
			add(("" + this.ref() + ".render_(" + item2.c(o) + "," + (slot++) + ")"));
		};
	};
...
Tag.prototype.jsold = function (jso){
	var o = this._options;
