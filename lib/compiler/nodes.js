...
Stack.prototype.optlevel = function (){
	return (this._optlevel == null) ? (this._optlevel = ((this._options.conservative || this.env('IMBA_CONSERVATIVE')) ? 0 : 9)) : this._optlevel; // stack.option(:conservative)
};
...
TagLoopFunc.prototype.visit = function (stack){
	...
	this.set({treeType: (len$(this._tags) == 0) ? 3 : 5});
	return this;
};
...
TagLoopFunc.prototype.capture = function (node){
	...
	var fn = key ? 'createTagMap' : 'createTagList';
	var get = CALL(this.scope__().imbaRef(fn),this._tag.cacheRef() ? [gen,oref,this._tag.cacheRef()] : [gen,oref]);
...
Tag.prototype.visit = function (stack){
	...
	if ((scope instanceof ForScope) && o.par) {
		if (o.par._tagScope != scope) {
	...
	if ((o.body instanceof ListNode) && stack.optlevel() > 0) { // and stack.env('TAG_OPTIM')
...
Tag.prototype.cacher = function (){
	var op;
	return (this._options.cacher == null) ? (this._options.cacher = this.parent() ? (
		this.parent().childCacher()
	) : ((this._options.ivar || this._options.key) ? (
		op = OP('||=',OP('.',new This(),'$$'),LIT('{}')),
		new TagCache(this,this._tagScope.closure().declare("$",op,{system: true,type: 'let'}))
	) : (
		null
	))) : this._options.cacher;
};

Tag.prototype.childCacher = function (){
	var scop, nr, meth, key, ctor, op;
	return this._options.childCacher || (this._options.childCacher = (this._fragment || this.isSelf()) ? (
		scop = this._tagScope.closure(),
		nr = scop.incr('selfTag'),
		meth = (scop instanceof MethodScope) ? scop.node().name() : '',
		key = ("$" + nr),
		(meth && meth != 'render') && (
			key += meth
		),
		...
		// let op = OP('.',This.new,'$')
		this._typeNum = new Num(2),
		op = OP('||=',OP('.',new This(),key),CALL(ctor,[new This(),this._typeNum])),
		// TagCache.new(self,@tagScope.closure.declare("$",op,system: yes))
		new TagCache(this,this._tagScope.closure().declare("$",OP('.',new This(),'$'),{system: true}))
	) : ((!(this.parent()) || this._options.ownCache) ? (
		// if it has no parent we force a reference
		new TagCache(this,OP('.',this.reference(),'$'))
	) : (
		this.parent().childCacher()
	)));
};
...
Tag.prototype.cacheRef = function (){
	...
	return o.treeRef = (o.ivar ? new Str("'" + o.ivar.c() + "'") : this.cacher().nextRef(this));
};
...
Tag.prototype.js = function (jso){
	...
	if (this.isSelf()) {
		...
		if (o.ivar) {
			...
			if (!o.optim || o.optim == this || parentType != 2) {
				pre = ("" + (o.path) + " || ");
			};
		};
	...
	if (o.body instanceof Func) {
		...
		if (len$(children) == 1) {
			...
			contentType = children.every(function(item) {
				return (item instanceof Tag) || item.option('treeType') == 2 || item.isPrimitive();
			}) ? 2 : 1;
	...
	if (!(this.isNative()) || o.template || calls.length > 0 || this._children.length) {
		var commits = this._children.map(function(child) { return child.option('commit'); }).filter(function(item) { return item; });
		var args = (o.optim && commits.length) ? ('(' + INDENT.wrap(commits.join(',\n')) + ',true)') : '';
	...
	if (calls != statics) {
		if (o.optim && o.optim != this) {
			...
			if (calls.length) { this.set({commit: ("" + (o.path) + calls.join(""))}) };
		} else {
			out = ("(" + out + ")") + calls.join("");
		};
	};
	...
	if (this.isSelf() && false) {
		...
		for (var j = 0, ary = iter$(this.childCacher()._refs), len_ = ary.length; j < len_; j++) {
			var io = ary[j].node()._options;
			var typ2 = io.treeType;
			var calls1 = io.calls;
			console.log("found ref",typ2);
			if (io.calls) {
				apply.push(io.path + calls1.join(''));
			};
		};
