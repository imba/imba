...
For.prototype.js = function (o){
	...
	if (this._tag && this._childTags) {
		...
		after += ";c$=c$0";
		after += ";t$=t$0";
		if (indexed) { after += ";f$.reconcile(f$i)" };
		if (!indexed) { after += (";" + (this._tag.ref()) + ".render_(" + this.ref() + "," + (this._slot || '0') + ")") };
	};
...
Tag.prototype.js = function (o){
	...
	if (!this._parent) {
		...
		if (this.isSelf()) {
			...
			add(("t$ = this." + this.oid() + "$ || (this." + this.oid() + "$ = " + this.create_() + "(" + ctor.join(',') + "))"));
		};
		add("c$ = t$.$");
		add("b$ = c$._");
		// add "{ref}=t$"
		...
		add("b$=1");
		add(("" + this.ref() + " || (b$=0," + this.ref() + " = " + this.create_() + "(" + ctor.join(',') + "))"));
	};
	...
	for (var j = 0, ary = iter$(this._attributes), len_ = ary.length, item1; j < len_; j++) {
		...
		if (item1.isStatic()) {
			...
			var js = ("b$ || (" + this.ref() + "." + item1.c(o) + ")");
			add(js);
		} else {
			var val = item1.value();
			item1.setValue(LIT(("" + (item1.ref()) + "=v$")));
			add(("v$=" + val.c(o)));
			add(("v$===" + (item1.ref()) + " || (" + this.ref() + "." + item1.js(o) + ")"));
		};
	};
	...
	for (var k = 0, len__ = nodes.length, item2; k < len__; k++) {
		...
		if (item2 instanceof Tag) {
			item2._slot = slot++;
			add(item2.c(o));
		} else if (item2 instanceof Str) {
			// should do a basic init check
			add(("b$ || " + this.ref() + ".render_(" + item2.c(o) + "," + (slot++) + ")"));
			...
			add(("v$===c$." + id + " || " + this.ref() + ".render_(c$." + id + "=v$," + (slot++) + ")"));
