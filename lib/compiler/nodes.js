(function(){
	...
	function mark__(tok){
		if (tok && OPTS.map && tok.sourceMapMarker) {
	...
	Terminator.prototype.c = function (){
		...
		// and fix it up
		return mark__(this._value) + this._value.c();
		// var v = value.replace(/\\n/g,'\n')
		// v # .split()
		// v.split("\n").map(|v| v ? " // {v}" : v).join("\n")
	};
	...
	Root.prototype.compile = function (o){
		...
		if (o.map) {
			var mapper = new SourceMap(result);
			mapper.generate();
		};
	...
	Assign.prototype.js = function (o){
		...
		this.p(("typeof op " + (this._op.constructor)));
		var out = ("" + (l.c()) + " " + (mark__(this._op)) + this.op() + " " + (this.right().c({expression: true})));
