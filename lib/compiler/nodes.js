...
__root.parseError = function (str,o){
	// find nearest token
	var err;
	...
	e.options = o;
	throw e;
}; exports.parseError = __root.parseError;

__root.C__ = function (obj){
	return (typeof obj == 'string') ? obj : obj.c();
};

__root.MARK__ = function (tok){
...
__root.REDUCE__ = function (res,ary){
	for (var i = 0, items = iter$(ary), len = items.length, v1; i < len; i++) {
		v1 = items[i];
		(v1 instanceof Array) ? __root.REDUCE__(res,v1) : res.push(v1);
	};
	return;
};

__root.FLATTEN__ = function (ary,compact){
	if(compact === undefined) compact = false;
	var out = [];
	for (var i = 0, items = iter$(ary), len = items.length, v1; i < len; i++) {
		v1 = items[i];
		(v1 instanceof Array) ? __root.REDUCE__(out,v1) : out.push(v1);
	};
	return out;
};
...
__root.CounterToShortRef = function (nr){
...
ListNode.prototype.compact = function (){
	this._nodes = __root.COMPACT__(this._nodes);
	return this;
};
...
VarBlock.prototype.js = function (o){
	var code = __root.COMPACT__(__root.FLATTEN__(__root.CARY__(this.nodes())));
...
Parens.prototype.js = function (o){
	...
	if (par instanceof Block) {
		...
		str = (v instanceof Array) ? __root.CARY__(v) : v.c({expression: o.isExpression()});
	} else {
		str = (v instanceof Array) ? __root.CARY__(v) : v.c({expression: true});
	};
...
ParamList.prototype.js = function (o){
	...
	if (o.parent() instanceof Code) {
		...
		return __root.COMPACT__(pars.map(function(arg) { return __root.C__(arg.varname()); })).join(",");
	} else {
		throw "not implemented paramlist js";
		return "ta" + __root.COMPACT__(this.map(function(arg) { return arg.c(); })).join(",");
	};
};
...
VariableDeclaration.prototype.js = function (o){
	...
	// FIX PERFORMANCE
	var out = __root.COMPACT__(__root.CARY__(this.nodes())).join(", ");
...
ClassDeclaration.prototype.js = function (o){
	...
	var mark = __root.MARK__(this.option('keyword'));
...
TagLoopFunc.prototype.js = function (o){
	...
	return "(" + out + (")(" + __root.CARY__(this._args) + ")");
};
...
MethodDeclaration.prototype.visit = function (){
	...
	// find closest block
	var up_, variable;
	console.log(("collect decorators? " + this.up() + " " + (this.up()._decorators)));
	this._decorators = (up_ = this.up()) && up_.collectDecorators  &&  up_.collectDecorators();
	
	if (this._decorators) {
		console.log("found decorators for method!!!",this._decorators);
	};
	...
	if (!this._target) {
		...
		if (this.context() instanceof RootScope) {
			this.set({static: true,root: true});
			this._target = this.context().context().reference();
			// change the inner context of this scope?
			this.scope()._context = this.context().context();
			// scope.context.@reference = @target # scope.declare("self",op)
			this._variable = this.context().register(this.name(),this,{type: 'meth'});
			this._variable.proxy(OP('.',this._target,this.name()));
		} else {
			this._variable = this.context().register(this.name(),this,{type: 'meth'});
		};
	};
...
MethodDeclaration.prototype.js = function (o){
	...
	var out = "";
	var mark = __root.MARK__(this.option('def'));
	var fname = __root.SYM__(this.name());
	...
	if (this.option('inObject')) {
		...
		fref = ("" + (target.c()) + "." + fname);
		out = ("" + mark + fref + " = " + this.funcKeyword() + " " + func);
	} else if (target) {
	...
	if (this.option('export')) {
		out = ("" + out + "; exports." + (this.option('default') ? 'default' : fname) + " = " + fref + ";");
...
Undefined.prototype.c = function (){
	return __root.MARK__(this._value) + "undefined";
};
...
Nil.prototype.c = function (){
	return __root.MARK__(this._value) + "null";
};
...
True.prototype.c = function (){
	return __root.MARK__(this._value) + "true";
};
...
False.prototype.c = function (){
	return __root.MARK__(this._value) + "false";
};
...
Num.prototype.c = function (o){
	...
	return paren ? (("(" + __root.MARK__(this._value)) + js + ")") : ((__root.MARK__(this._value) + js));
...
Symbol.prototype.raw = function (){
	return this._raw || (this._raw = __root.SYM__(this.value().toString().replace(/^\:/,'')));
};

Symbol.prototype.js = function (o){
	return ("'" + __root.SYM__(this.raw()) + "'");
};
...
Arr.prototype.js = function (o){
	...
	if (splat) {
		...
		return ("[].concat(" + __root.CARY__(slices).join(", ") + ")");
	} else {
		...
		//	"[\n{value.c.join(",\n").indent}\n]"
		var out = (val instanceof Array) ? __root.CARY__(val) : val.c();
		return ("[" + out + "]");
	};
};
...
Op.prototype.js = function (o){
	...
	if (l && r) {
		out = ("" + l + " " + __root.MARK__(this._opToken) + op + " " + r);
	} else if (l) {
		out = ("" + __root.MARK__(this._opToken) + op + l);
	};
...
ComparisonOp.prototype.js = function (o){
	...
	if (l instanceof Node) { l = l.c() };
	if (r instanceof Node) { r = r.c() };
	return ("" + l + " " + __root.MARK__(this._opToken) + op + " " + r);
};
...
UnaryOp.prototype.isTruthy = function (){
	var val = __root.TRUTHY__(this.left());
...
InstanceOf.prototype.js = function (o){
	...
	if (this.right() instanceof Const) {
		...
		var name = __root.C__(this.right().value());
...
VarOrAccess.prototype.visit = function (){
	...
	if (variable && variable.declarator()) {
		...
		if (variable && variable._initialized || (scope.closure() != vscope.closure())) {
			...
			if ((vscope instanceof RootScope) && vscope.context() != scope.context() && variable.type() == 'meth') {
				this.warn(("calling top-level " + this.value() + " is deprecated - use __root." + this.value()));
			};
...
VarOrAccess.prototype.c = function (){
	return __root.MARK__(this._token) + (this._variable ? VarOrAccess.__super__.c.call(this) : this.value().c());
};
...
VarReference.prototype.js = function (o){
	...
	var out = ("" + __root.MARK__(this._value) + (ref.c()));
	var keyword = o.es5() ? 'var' : ((this._type || 'var'));
...
Assign.prototype.js = function (o){
	...
	if (l instanceof PropertyAccess) {
		...
		if (this.isUsed()) {
			...
			ast = new Parens(__root.BLK__([ast,this.right()]));
		};
	...
	if (this.option('export')) {
		...
		return ("" + lc + " " + __root.MARK__(this._opToken) + this.op() + " exports." + ename + " = " + this.right().c({expression: true}));
	} else {
		return ("" + lc + " " + __root.MARK__(this._opToken) + this.op() + " " + this.right().c({expression: true}));
	};
	// return out
};
...
Identifier.prototype.symbol = function (){
	...
	return this._symbol || (this._symbol = __root.SYM__(this.value()));
};
...
Identifier.prototype.setter = function (){
	...
	return this._setter || (this._setter = true && (
		tok = new Token('IDENTIFIER',__root.SYM__('set-' + this._value),this._value._loc || -1),
		new Identifier(tok)
...
Identifier.prototype.alias = function (){
	return __root.SYM__(this._value);
};
...
Const.prototype.c = function (){
	if (this.option('export')) {
		return ("exports." + (this._value) + " = ") + __root.MARK__(this._value) + this.js();
	} else {
		return __root.MARK__(this._value) + this.js();
	};
};
...
If.prototype.visit = function (stack){
	...
	if (!stack.isAnalyzing()) {
		this._pretest = __root.TRUTHY__(this.test());
...
If.prototype.js = function (o){
	...
	if (o.isExpression()) {
		...
		var out = ("" + __root.MARK__(this._type) + "if (" + cond + ") ") + code; // ' {' + code + '}' # '{' + code + '}'
...
Loop.prototype.addBody = function (body){
	this.setBody(__root.BLK__(body));
	return this;
};
...
For.prototype.js = function (o){
	...
	var head = ("" + __root.MARK__(this.options().keyword) + "for (" + (this.scope().vars().c()) + "; " + cond.c({expression: true}) + "; " + final.c({expression: true}) + ") ");
	return head + code;
};
...
ForOf.prototype.js = function (o){
	...
	if (this.options().own) {
		...
		var head = ("" + __root.MARK__(this.options().keyword) + "for (" + (this.scope().vars().c()) + "; " + (OP('<',i,vars.len).c()) + "; " + (OP('++',i).c()) + ")");
		return head + code;
	} else {
		if (v && v.refcount() > 0) {
			this.body().unshift(OP('=',v,OP('.',src,k)));
		...
		return ("" + __root.MARK__(this.options().keyword) + "for (" + (o.es5() ? 'var' : 'let') + " " + (k.c()) + " in " + inCode.c({expression: true}) + ")") + code;
	};
};
...
// NO NEED?
function Begin(body){
	this._nodes = __root.BLK__(body).nodes();
};
...
Switch.prototype.js = function (o){
	...
	return ("switch (" + (this.source().c()) + ") ") + helpers.bracketize(__root.CARY__(body).join("\n"),true);
};
...
TagCache.prototype.nextValue = function (){
	var ref = __root.CounterToShortRef(this._counter++);
...
TagCache.prototype.nextSlot = function (){
	...
	if (this._options.keys) {
		return new Str("'" + __root.CounterToShortRef(this._index - 1) + "'");
	} else {
...
TagHandler.prototype.js = function (o){
	...
	// find the context
	return ("on$(" + this.slot() + ",[" + __root.CARY__(parts) + "]," + (this.scope__().context().c()) + ")");
...
Tag.prototype.js = function (jso){
	...
	for (var i = 0, items = iter$(this._attributes), len = items.length, part; i < len; i++) {
		...
		out1 = ("." + __root.MARK__(part.name())) + out1;
		// if part.isSpecial
		//	specials.push(out)
		if (part.isStatic()) {
			statics.push(out1);
...
TagTree.prototype.load = function (list){
	if (list instanceof ListNode) {
		...
		return __root.COMPACT__((list instanceof Array) ? list : [list]);
	};
};
...
SelectorAttribute.prototype.c = function (){
	...
	if (this._right instanceof Str) {
		...
		return ("[" + (this._left.c()) + (this._op) + "\"'+" + __root.C__(this._right) + "+'\"]");
	} else {
		return ("[" + (this._left.c()) + "]");
...
Util.slice = function (obj,a,b){
	...
	return CALL(OP('.',obj,slice),__root.COMPACT__([a,b]));
};
...
Util.Extend.prototype.js = function (o){
	// When this is triggered, we need to add it to the top of file?
	return ("extend$(" + __root.COMPACT__(__root.CARY__(this.args())).join(',') + ")");
};
...
Scope.prototype.dump = function (){
	...
	var vars = Object.keys(self._varmap).map(function(k) {
		...
		// DUMP__(v)
		return v.references().length ? __root.DUMP__(v) : null;
	});
...
RootScope.prototype.dump = function (){
	var obj = {warnings: __root.DUMP__(this._warnings)};
...
Variable.prototype.dump = function (typ){
	...
	return {
		type: this.type(),
		name: name,
		refs: __root.DUMP__(this._references,typ)
	};
};
...
RootScopeContext.prototype.reference = function (){
	return this._reference || (this._reference = this.scope().declare("__root",this.scope().object(),{type: 'global'}));
};
