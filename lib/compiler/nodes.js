...
if (true) {
	let v = (process.version || 'v0').match(/^v?(\d+)/);
...
Block.prototype.visit = function (stack){
	...
	if (stack && stack._tag && !stack._tag._tagLoop) {
		...
		if (real.length > 1) {
			let nr = this._tag.childCacher().nextBlock(); //  @tag.blocks.push(self)
...
VarList.prototype.visit = function (){
	...
	var r;
	for (let i = 0, items = iter$(this.left()), len = items.length; i < len; i++) {
...
VarList.prototype.js = function (o){
	...
	if (ll > 1 && rl == 1) {
		...
		var r = this.right()[0];
		r.cache();
		for (let i = 0, items = iter$(this.left()), len = items.length, l; i < len; i++) {
			l = items[i];
			if (l.splat()) {
				throw "not supported?";
...
ClassDeclaration.prototype.js = function (o){
	...
	for (let i = 0, items = iter$(head.reverse()), len = items.length; i < len; i++) {
...
ModuleDeclaration.prototype.visit = function (){
	...
	if (this._name) {
		let modname = String(this.name()._value || this.name());
...
TagDeclaration.prototype.js = function (o){
	...
	if (this.superclass()) {
		...
		let supname = this.superclass().name();
...
TagLoopFunc.prototype.visit = function (stack){
	...
	if (len$(this._tags) == 1 && !single.option('key')) {
		...
		if (this._isFast) {
			let len = this._loop.options().vars.len;
			if (len && len.declarator) {
				let defs = len.declarator().defaults();
				len.declarator().setDefaults(OP('=',OP('.',this._params.at(0),'taglen'),defs));
			};
	...
	if (!this._isFast) {
		for (let i = 0, items = iter$(this._tags), len = items.length; i < len; i++) {
			items[i]._loopCache._callee = this.scope__().imbaRef('createTagMap');
		};
	};
	
	if (!this._parentLoop) {
		let collectInto = new ValueNode("val");
		let collector = new TagPushAssign("push",collectInto,null);
		this._loop.body().consume(collector);
		let collected = collector.consumed();
		let treeType = 3;
		
		if (len$(collected) == 1 && (collected[0] instanceof Tag)) {
			let op = CALL(OP('.',this._params.at(0),'$iter'),[]);
			...
			if (len && len.declarator) {
				let defs = len.declarator().defaults();
				len.declarator().setDefaults(OP('=',OP('.',this._resultVar,'taglen'),defs));
			};
		} else {
...
TagLoopFunc.prototype.capture = function (node){
	...
	if (o.loop == this) {
		...
		let gen = o.loopCacher || this._tag.childCacher();
		let oref = o.parRef || this._tag.childCacher().nextRef();
		
		let nr = this._tags.push(node) - 1;
		let ref = this._loop.option('vars').index;
		let param = this._params.at(this._params.count(),true,("$" + (this._params.count())));
		...
		let parentRef = this._tag.cachePath(); // will be correct even if nested loops
		
		let get = CALL(this.scope__().imbaRef(fn),parentRef ? [gen,oref,parentRef] : [gen,oref]);
		node._loopCache = get;
		...
		let ref = this._loop.option('vars').index;
		let param = this._params.at(this._params.count(),true,("$" + (this._params.count())));
		param.setVariable(this.scope__().register(("$" + (this._params.count())),this,{system: true}));
...
PropertyDeclaration.prototype.c = function (){
	...
	if (isAttr) { // (@token and String(@token) == 'attr') or o.key(:dom) or o.key(:attr)
		let attrKey = (o.key('dom') instanceof Str) ? o.key('dom') : this.name().value();
...
Arr.prototype.js = function (o){
	...
	if (splat) {
		...
		for (let i = 0, items = iter$(nodes), len = items.length, v; i < len; i++) {
			v = items[i];
...
Assign.prototype.js = function (o){
	...
	if (this.option('export')) {
		let ename = (l instanceof VarReference) ? l.variable().c() : lc;
...
TupleAssign.prototype.js = function (o){
	...
	if (o.isExpression() && self._vars) {
		for (let i = 0, items = iter$(self._vars), len_ = items.length; i < len_; i++) {
			items[i].variable().autodeclare();
		};
	} else if (self._vars) {
		for (let i = 0, items = iter$(self._vars), len_ = items.length; i < len_; i++) {
			items[i].variable().predeclared();
		};
	};
...
Call.prototype.js = function (o){
	...
	if (splat) {
		...
		out = ("" + callee.c({expression: true}) + ".apply(" + (rec.c()) + "," + ary.c({expression: true}) + ")");
...
Loop.prototype.tagvar = function (name){
	...
	return this._tagvars[name] || (this._tagvars[name] = this.scope__().closure().temporary(null,{reuse: true},("t" + name + "$" + (this._level))));
};
...
For.prototype.js = function (o){
	...
	if (this._tag && this._childTags) {
		let iref = this.scope__().imbaRef('createFragment','fragment$').c();
		...
		this.option('indexed',!!indexed);
		
		// should know how many inner slots this fragment has?
		before += ("f$ = " + this.ref() + " || (" + this.ref() + " = " + iref + "(" + typ + "," + (this._tag.ref()) + "," + (this._slot || '0') + "));\n");
		this._ref = "f$";
		before += "f$i = 0;\n";
		before += "c$0=c$;\n";
		before += "t$0=t$;\n";
		...
		if (indexed) {
			...
			after += (";" + (this._tag.ref()) + ".render$(" + this.ref() + "," + (this._slot || '0') + ")");
		};
	};
...
ForOf.prototype.js = function (o){
	...
	if (this.options().own) {
		...
		let inCode = osrc._variable ? src : ((OP('=',src,osrc)));
...
TagCache.prototype.nextRef = function (node){
	let item = new TagCacheKey(this,node);
	this._refs.push(item);
	return item;
};

TagCache.prototype.nextValue = function (){
	let ref = AST.counterToShortRef(this._counter++);
...
TagAttr.prototype.js = function (){
	...
	let dyn = this._chain.length || this._tag.type()._ns;
	let val = this.value() ? this.value().js() : this.quoted();
	let add = '';
...
TagModifier.prototype.visit = function (){
	...
	for (let i = 0, items = iter$(this._params), len = items.length, param; i < len; i++) {
		...
		if (param.value() instanceof GlobalVarAccess) {
			let special = new TagHandlerSpecialArg(param.value().c());
...
TagHandler.prototype.js = function (o){
	let parts = [this.quoted()].concat(this._chain);
...
Tag.prototype.addPart = function (part,type){
	let curr = this._attributes.CURRENT;
...
Tag.prototype.fragment = function (){
	return this._fragment = (this.isSelf() || !this._parent || (this._parent instanceof Loop)) ? this : ((this._parent.fragment()));
};
...
Tag.prototype.js = function (o){
	...
	let typ = this.isSelf() ? "self" : ((this.type().isClass() ? this.type().name() : ("'" + this.type()._value + "'")));
	...
	if (!this._parent) {
		...
		if (this.isSelf()) {
			...
			// add "var {vars}"
			add(("" + this.bvar() + " = 1"));
			add(("" + this.tvar() + " = this." + this.oid() + "$$ || (" + this.bvar() + "=0,this." + this.oid() + "$$ = " + ctor + ")"));
			add(("" + this.cvar() + " = " + this.tvar() + ".$ || (" + this.bvar() + "=0," + this.tvar() + ".$=\{\})"));
		...
		add(("t$ = " + this.ref() + " || (b$=0," + this.ref() + " = " + ctor + ")"));
		this._ref = "t$";
		add(("c$=t$.$ || (t$.$=\{\})"));
	} else {
		
		if (nodes.length == 0 && dynamics.length == 0 && dynamicFlags.length == 0) {
			canInline = true;
		};
		...
		if (canInline) {
			...
			for (let i = 0, len = statics.length; i < len; i++) {
				res.push(("" + this.tvar() + "." + statics[i].c(o)));
			};
			var parts = res;
			if (parts.length > 0) { parts.unshift('') };
			...
			let ref = ("" + (this.fragment().cvar()) + "." + this.oid());
	...
	for (let i = 0, items = iter$(this._attributes), len = items.length, item; i < len; i++) {
		...
		item = items[i];
		if (item.isStatic()) {
			if (item instanceof TagFlag) { continue; };
			let js = ("b$ || (" + this.ref() + "." + item.c(o) + ")");
			add(js);
		} else {
			let val = item.value();
			item.setValue(LIT(("" + (item.ref()) + "=v$")));
			add(("v$=" + val.c(o)));
			
			if ((item instanceof TagFlagExpr) && optimizeFlag) {
				add(("v$===" + (item.ref()) + " || " + this.ref() + ".flag$(" + item.value().c(o) + ")"));
			} else if (item instanceof TagFlag) {
				if (optimizeFlag) {
					add(("v$===" + (item.ref()) + " || " + this.ref() + ".flag$(v$ ? " + (item.quoted()) + " : ''," + item.value().c(o) + ")"));
				} else {
					add(("v$===" + (item.ref()) + " || " + this.ref() + ".flagIf$(" + (item.quoted()) + "," + item.value().c(o) + ")"));
				};
			} else {
				add(("v$===" + (item.ref()) + " || (" + this.ref() + "." + item.js(o) + ")"));
			};
		};
	};
	...
	for (let i = 0, len = nodes.length, item; i < len; i++) {
		...
		if (item instanceof Tag) {
			...
			item._slot = slot++;
			add(item.c(o));
		} else if (item instanceof Str) { // static for sure
			// should do a basic init check
			add(("b$ || " + this.ref() + ".insert$(" + item.c(o) + "," + (slot++) + ")"));
		} else {
			...
			if (count == 1 && (item instanceof InterpolatedString)) {
				add(("v$===" + key + " || " + this.ref() + ".text$(" + key + "=v$)"));
			} else if (count == 1) {
				add(("v$===" + key + " || " + this.ref() + ".insert$(" + key + "=v$,-1," + key + "_)"));
			} else {
				add(("v$===" + key + " || (" + key + "_ = " + this.ref() + ".insert$(" + key + "=v$," + (slot++) + "," + key + "_))"));
			};
		};
	};
	...
	if (this._parent instanceof Loop) {
		...
		if (this._parent.option('indexed')) {
			...
			add(("" + (this._parent.ref()) + ".push(" + this.ref() + ",f$i++)"));
		};
...
Selector.prototype.query = function (){
	...
	for (let i = 0, items = iter$(this.nodes()), len = items.length; i < len; i++) {
		var val = items[i].c();
