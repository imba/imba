...
MethodDeclaration.prototype.visit = function (){
	...
	if (!this._target) {
		...
		if (this.context() instanceof RootScope) {
			...
			this.scope().context()._reference = this._target; // scope.declare("self",op)
...
MethodDeclaration.prototype.js = function (o){
	...
	if (this.option('inObject')) {
		...
		fref = ("" + (target.c()) + "." + fname);
		out = ("" + mark + fref + " = " + this.funcKeyword() + " " + fname + func);
	} else if (target) {
...
TagHandler.prototype.js = function (o){
	...
	// find the context
	return ("on$(" + this.slot() + ",[" + cary__(parts) + "]," + (this.scope__().context().c()) + ")");
