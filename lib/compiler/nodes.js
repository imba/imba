...
TagDeclaration.prototype.tagspace = function (){
	var ctx;
	return ctx = this.scope().closure().tagContextPath();
...
TagLoopFunc.prototype.capture = function (node){
	...
	var factoryParams = [typ,oref];
	if (this._tag.childRef()) { factoryParams.push(this._tag.childRef()) };
	this._args.push(OP('||',OP('.',gen,oref),CALL(OP('.',gen,'$'),factoryParams)));
	return this;
};
...
Tag.prototype.visit = function (stack){
	...
	} else if ((!stack._tag && !o.key && !(this.reactive()) && !(this.isSelf()) && !o.ivar)) {
	...
	if (this.isSelf()) {
		...
		// let r = 
		var op = OP('||=',OP('.',new This(),'$$'),LIT('{}'));
		// OP('||',OP('.',This.new,'$$'),OP('=',scope.context,'$'),factoryParams))
		o.treeRef = o.key;
		o.key.cache();
		// what if we are in loop?
		this._trunk = new TagCache(this,scope.declare("$",op));
		...
		if (meth && false) {
			var key = "'" + meth.name() + "'"; // what if there are more?
			var op1 = CALL(OP('.',OP('.',new This(),'$'),'$$'),[key]);
			this._staticCache = new TagCache(this,scope.declare("$",op1)); // .set(lowercase: yes)
		} else {
			true;
		};
	};
...
Tag.prototype.makeFragment = function (closed){
	...
	var o = this._options;
	this._trunk = o.par ? o.par.staticCache() : null;
...
Tag.prototype.trunk = function (){
	return this._trunk || (this._options.par ? this._options.par.staticCache() : this.staticCache());
};
...
Tag.prototype.staticCache = function (){
	...
	if (this._fragment) {
		...
		return this._staticCache = new TagCache(this,this._fragment.scope().declare("$",OP('.',new This(),'$'))); // .set(lowercase: yes)
	} else if (this.type() instanceof Self) {
		return this._staticCache = new TagCache(this,this._tagScope.declare("$",OP('.',new This(),'$')));
		// @staticCache ||= @tagScope.tagContextCache
	} else if (this.option('loop')) { // weird case?
		throw "should never hit? staticCach loop";
		return this._staticCache = new TagCache(this,OP('.',this.reference(),'$'));
	} else if (this._options.par) {
		return this._staticCache = this._options.par.staticCache();
	} else if (this._parent) {
		return this._staticCache = this._parent.staticCache();
	} else {
		return this._staticCache = new TagCache(this,OP('.',this.reference(),'$'));
	};
};
...
Tag.prototype.cacheRef = function (){
	...
	return o.treeRef = (o.ivar ? new Str("'" + o.ivar.c() + "'") : this.trunk().nextRef());
};

Tag.prototype.childRef = function (){
	return (this._options.loop || this._options.factory) ? null : this.cacheRef();
};
...
Tag.prototype.js = function (jso){
	...
	if (this.isSelf()) {
		...
		// if o:template
		return ("" + (scope.imbaTags()) + ".$(" + typ + "," + o.body.c({expression: true}) + ").end()");
	} else if (o.ivar && o.factory) {
		// at the root
		o.path = OP('.',scope.context(),o.ivar).c();
		out = ("" + (o.path) + "=" + (o.path) + "||" + (scope.imbaTags()) + ".$(" + typ + "," + (scope.context().c()) + ").ref_('" + o.ivar._value.slice(1) + "')");
	} else if (o.factory) {
		this;
	} else {
		...
		if (o.ivar) {
			...
			out = ("" + (this.trunk().c()) + "[" + (ref.c()) + "]||");
		};
		
		
		if (o.par && o.par.childRef()) {
			out += ("" + (this.factory().c()) + "(" + typ + "," + (this.trunk().c()) + "," + (ref.c()) + "," + (o.par.childRef().c()) + ")");
			// out += "{trunk.c}.$({typ},{ref.c},{o:par.childRef.c})"
		} else if (ref && this.trunk()) {
			out += ("" + (this.trunk().c()) + ".$(" + typ + "," + (ref.c()) + ")");
		} else {
			if (o.ivar && o.path) {
				out += o.path + "=";
			};
