(function(){
	...
	Tag.prototype.visit = function (){
		this.scope__().root().requires('imba');
	...
	RootScope.prototype.requires = function (path,name){
		...
		if (name) {
			var val = this._requires[path] = this.declare(name || "unnamed",req);
			return val;
		} else {
			this._head.push(this._requires[path] = req);
			return req;
		};
	};
