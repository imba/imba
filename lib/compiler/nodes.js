...
function reduce__(res,ary){
	for (var i = 0, items = iter$(ary), len = items.length, v1; i < len; i++) {
		v1 = items[i];
		(v1 instanceof Array) ? reduce__(res,v1) : res.push(v1);
	};
	return;
};
...
function flatten__(ary,compact){
	...
	for (var i = 0, items = iter$(ary), len = items.length, v1; i < len; i++) {
		v1 = items[i];
		(v1 instanceof Array) ? reduce__(out,v1) : out.push(v1);
	};
	return out;
};
...
ModuleDeclaration.prototype.visit = function (){
	...
	if (this._name) {
		...
		this.scope().parent().register(modname,this,{type: 'var'});
...
ModuleDeclaration.prototype.js = function (o){
	this.scope().context().setValue(this._ctx = this.scope().declare('$module',null,{system: true}));
...
TagLoopFunc.prototype.visit = function (stack){
	...
	this._tags = [];
	this._args = [];
	TagLoopFunc.__super__.visit.apply(this,arguments);
	...
	if (len$(this._tags) == 1 && !this._tags[0].option('key')) {
		console.log("CAN OPTIMIZE LOOP!");
		var lenDecl = this._loop.options().vars.len.declarator();
		if (lenDecl) {
			lenDecl.setDefaults(OP('=',OP('.',this._params.at(0),'taglen'),lenDecl.defaults()));
		};
		this._body.push(new Return(this._params.at(0)));
	} else {
		this._resultVar = this._params.at(len$(this._tags),true,'$$'); // visiting?
		this._resultVar.visit(stack);
		var collector = new TagPushAssign("push",this._resultVar,null);
		this._loop.body().consume(collector);
		this._body.push(new Return(this._resultVar));
...
TagLoopFunc.prototype.capture = function (node){
	...
	node.set({par: null});
	var gen = this._tag.staticCache();
	var oref = gen.nextRef();
	var nr = this._tags.push(node) - 1;
	var ref = this._loop.option('vars').index;
	var key = node.option('key');
	var param = this._params.at(nr,true,'$' + nr);
	// param.visit
	node._staticCache = new TagCache(this,param);
	...
	var typ = new Num(key ? 5 : 4);
	this._args.push(OP('||',OP('.',gen,oref),CALL(OP('.',gen,'$'),[typ,oref,this._tag.cacheRef()])));
	return this;
};
...
MethodDeclaration.prototype.visit = function (){
	...
	if (this.context() instanceof ClassScope) {
		...
		if (this.context() instanceof ModuleScope) {
			this.body().set({strict: true});
			// let op = OP('==',This.new,LIT('window'))
			var op = OP('||',new This(),this.context().context());
			this.scope().context()._reference = this.scope().declare("self",op);
		};
	};
...
MethodDeclaration.prototype.js = function (o){
	...
	if (this.option('inObject')) {
	...
	} else if ((ctx instanceof ClassScope) && !target) {
		if (this.type() == 'constructor') {
			out = ("" + mark + this.funcKeyword() + " " + fname + func);
		} else if (this.option('static') || (ctx instanceof ModuleScope)) {
			out = ("" + mark + (ctx.context().c()) + "." + fname + " = " + this.funcKeyword() + " " + func);
		} else {
			out = ("" + mark + (ctx.context().c()) + ".prototype." + fname + " = " + this.funcKeyword() + " " + func);
		};
		...
		out = ("" + mark + this.funcKeyword() + " " + fdecl + func);
	} else if (target && this.option('static')) {
		out = ("" + mark + (target.c()) + "." + fname + " = " + this.funcKeyword() + " " + func);
	} else if (target) {
		out = ("" + mark + (target.c()) + ".prototype." + fname + " = " + this.funcKeyword() + " " + func);
	} else {
		out = ("" + mark + this.funcKeyword() + " " + fdecl + func);
	};
...
VarOrAccess.prototype.visit = function (){
	...
	if (variable && variable.declarator()) {
		...
		if (variable.scope() == scope && !variable._initialized) {
		...
		if (variable && variable._initialized || (scope.closure() != variable.scope().closure())) {
...
For.prototype.traverse = function (){
	...
	if (stack._tag && !this._scope._tagLoop) {
		console.log("For loop inside tag(!)");
		var fn = this._scope._tagLoop = new TagLoopFunc([],[this]);
		stack.current().swap(this,fn);
		return fn.traverse();
	} else {
		return For.__super__.traverse.apply(this,arguments);
	};
};
...
For.prototype.singleTagInBody = function (){
	var singleTag = null;
	for (var i = 0, items = iter$(this.body()._nodes), len = items.length, child; i < len; i++) {
		child = items[i];
		if ((child instanceof Tag) && !singleTag) {
			singleTag = child;
...
For.prototype.consume = function (node){
	...
	if (node instanceof TagTree) {
		...
		var canOptimize = vars.len && !vars.diff && !vars.step && !this.options().guard && !(this.options().source instanceof Range);
		var singleTag1 = this.singleTagInBody();
		
		if ((this instanceof ForOf) && !this.options().own) {
		...
		if (singleTag1 && singleTag1.option('key')) {
			var key = singleTag1.option('key');
			if (key._variable != vars.index) {
				// let cacheVar = scope.register('$',null,system: yes, type: 'var')
				// wrong node?!
				node.set({treeType: TREE_TYPE.LOOP,loopVar: singleTag1.explicitCacheVar(this.scope()),cacheType: '$set'});
				singleTag1 = null;
			} else {
				singleTag1.set({key: vars.index});
			};
		};
		
		
		if (singleTag1 && canOptimize && STACK.platform() == 'web' && vars.len.declarator()) {
			node.set({treeType: TREE_TYPE.OPTLOOP});
			var cacheVar = singleTag1.explicitCacheVar(this.scope()); // scope.register('$',null,system: yes, type: 'var')
			var lenDecl = vars.len.declarator();
			if (lenDecl) {
				lenDecl.setDefaults(OP('=',OP('.',cacheVar,'taglen'),lenDecl.defaults()));
			};
			singleTag1.set({loopVar: cacheVar,loopIndex: vars.index});
			block = [this,new Return(cacheVar)];
			node.set({staticloop: true});
		};
		
		node._loop = null;
		
		var fn = new Lambda([],block);
	...
	var singleTag = this.singleTagInBody();
	if (singleTag) {
		singleTag.set({cacheType: '$set',loop: this,treeType: TREE_TYPE.LOOP});
	...
	if (node) {
		...
		var block1 = [this,BR,resvar.accessor().consume(node)];
		
		
		if (this._resvar) {
			block1.unshift(BR);
			block1.unshift(OP('=',new VarReference(this._resvar,'let'),resval));
		};
		
		ast = new Block(block1);
		return ast;
	};
...
ForOf.prototype.js = function (o){
	...
	if (this.options().own) {
		...
		return ("" + mark__(this.options().keyword) + "for (" + (o.es5() ? 'var' : 'let') + " " + (k.c()) + " in " + inCode.c({expression: true}) + ")") + code;
	};
};
...
TagCache.prototype.nextRef = function (){
	return new TagCacheKey(this);
};

TagCache.prototype.nextValue = function (){
	var ref = counterToShortRef(this._counter++);
...
TagFlag.prototype.js = function (){
	return this.value() ? (("flag(" + this.quoted() + "," + (this.value().c()) + ")")) : (("flag(" + this.quoted() + ")"));
};
...
TagData.prototype.isStatic = function (){
	return !(this.name()) || this.name().isPrimitive();
};

TagData.prototype.js = function (){
	return ("setData(" + (this.name().c()) + ")");
};
...
TagHandler.prototype.js = function (o){
	...
	return ("on$(" + this.slot() + ",[" + cary__(parts) + "])");
...
Tag.prototype.visit = function (stack){
	...
	if (typ == '->' || typ == '=>') {
		...
		this.makeFragment(typ == '->');
	} else if ((!stack._tag && !o.key && !(this.reactive()) && !(this.isSelf()))) {
		// why not just set all classes?
		o.isRoot = true;
		console.log(("make root! " + scope + " " + this.type() + " " + (scope._reactive)));
		var dynamics = this._attributes;
		var inner = new Tag({type: new This(),body: o.body,attributes: dynamics});
		this._attributes = [];
		var param = new RequiredParam(new Identifier('$$'));
		o.body = new TagFragmentFunc([param],Block.wrap([inner],[]),null,null,{closed: true});
		o.body.scope()._tagContext = o.body.scope().declare('$',OP('.',new This(),'$')); // param
	};
	
	// make sure we create a cache for this - or use the main one?
	if (this.isSelf()) {
		this._staticCache = new TagCache(this,scope.declare("$",OP('.',new This(),'$'))).set({lowercase: true});
	} else if (o.ivar && !prevTag) {
		// if this is a root ivar
		console.log("ivar cache");
		// access the real tag-cache? How do we know it is here?
		var key = "'" + STACK.method().name() + "'"; // what if there are more?
		var op = CALL(OP('.',OP('.',new This(),'$'),'$$'),[key]);
		this._staticCache = new TagCache(this,scope.declare("$",op)); // .set(lowercase: yes)
		this.setReactive(true);
	};
	
	// for scope should be wrapped immediately?
	if ((scope instanceof ForScope) && prevTag) {
		if (prevTag._tagScope != scope) {
			o.loop || (o.loop = scope._tagLoop);
			o.loop.capture(this);
			// @staticCache = prevTag.staticCache
			// scope.@tagParams ||= []
			// scope.@tagParams.push(cacheRef)
			// cacheRef.@type = 'loop'
			// set new cacheRef
			console.log("INSIDE LOOP! - not same as parent!!",this._staticCache);
			// cacheRef
		} else {
			// static-cache must be a reference to parent
			console.log("staticCache must be reference to parent");
			this._staticCache = new TagCache(this,OP('.',prevTag.reference(),'$'));
		};
	};
...
Tag.prototype.makeFragment = function (closed){
	if(closed === undefined) closed = false;
	if (this._fragment) { return this._fragment };
	var o = this._options;
	this._tree = new TagFragmentTree(this,o.body,{root: this,reactive: true});
	this._fragment = o.body = new TagFragmentFunc([],Block.wrap([this._tree]),null,null,{closed: closed});
...
Tag.prototype.staticCache = function (){
	...
	if (this._fragment) {
		...
		return this._staticCache || (this._staticCache = new TagCache(this,this._fragment.scope().declare("$",OP('.',new This(),'$')))); // .set(lowercase: yes)
	} else if (this.type() instanceof Self) {
		return this._staticCache || (this._staticCache = this._tagScope.tagContextCache());
	} else if (this.option('loop')) { // weird case?
		return this._staticCache || (this._staticCache = new TagCache(this,OP('.',this.reference(),'$'))); // .set(lowercase: yes)
	} else if (this._parent) {
		return this._staticCache || (this._staticCache = this._parent.staticCache());
	} else if (this._options.par) {
		return this._staticCache || (this._staticCache = this._options.par.staticCache());
	};
};

Tag.prototype.explicitCacheVar = function (scope){
	var options_;
	return this._explicitCacheVar = (options_ = this.options()).cacheVar || (options_.cacheVar = (scope || this._tagScope).register('$$',null,{system: true,type: 'var'}));
};
...
Tag.prototype.cacheRef = function (){
	var o = this._options;
	if (o.treeRef) { return o.treeRef };
	
	if (this.isSelf() || (!o.par && !o.ivar && !o.key)) {
		return null;
	};
	...
	return o.treeRef = (o.ivar ? new Str("'" + o.ivar.c() + "'") : this.staticCache().nextRef());
};
...
Tag.prototype.js = function (jso){
	...
	var c_zone = o.isRoot ? o.body.c({expression: true}) : scope.tagContext().c();
	var out = "";
	if (this.isSelf()) {
		commit = "synced";
		this.setReactive(true);
		...
		var gen = this.staticCache();
		var typ = this.type().isClass() ? this.type().name() : ("'" + this.type()._value + "'");
		...
		if (o.par && o.par.cacheRef()) { // there is always a parent?
	...
	if (tree) { tree.resolve() };
	
	for (var i = 0, items = iter$(this._attributes), len = items.length, part; i < len; i++) {
		part = items[i];
		var out1 = part.js(jso);
		out1 = ("." + mark__(part.name())) + out1;
		part.isStatic() ? statics.push(out1) : calls.push(out1);
	};
	
	// we need to trigger our own reference before the body does
	// but we do not need a reference if we have no body
	if (this.reactive() && tree && (o.loop) && ((o.body instanceof Func) || tree.hasTags())) {
		console.log("reference!!");
		this.reference();
	};
	
	if (this.reactive() && parent && parent.tree() && !o.ivar) {
		o.treeRef || (o.treeRef = parent.staticCache().nextRef());
	};
	...
	if (body) {
		...
		if (tree) {
			if (tree.option('treeType') && tree.single()) {
				typ1 = tree.option('treeType');
			} else if (tree.static()) {
				typ1 = 2;
			} else if (this.reactive() || tree.reactive()) {
				if (!tree.single() || (tree.single() instanceof If)) {
					typ1 = 1;
				} else if (tree.single() && tree.option('staticloop')) {
					typ1 = 4;
				} else {
					typ1 = 3;
				};
		...
		if (bodySetter == 'setChildren' || bodySetter == 'setContent') {
			calls.push(("." + bodySetter + "(" + body + "," + typ1 + ")"));
		} else if (bodySetter == 'setText') {
			var typ2 = (content instanceof Str) ? statics : calls;
			typ2.push(("." + bodySetter + "(" + body + ")"));
	...
	if ((o.ivar || o.key || this.reactive()) && !(this.type() instanceof Self)) {
		...
		if (o.ivar) {
			...
			var method = '_' + STACK.method().name();
			var parCache = OP('.',new Self(),'$');
			var paths = OP('.',parCache,method);
			var setter;
			var cache;
			...
			ctx = scope.closure().declare(this.explicitCacheVar(),new Parens(setter));
			key = o.key;
			...
			var pathKey = key;
			var path = OP('.',ctx,key);
			var kvar = this.explicitCacheVar(); // "${key}"
			
			var cache1 = LIT('{}');
			var setter1;
			
			if (o.key && o.key != o.loopIndex) {
				key = o.key; // ?
			} else if (o.loopVar) {
				kvar = o.loopVar;
				key = o.loopIndex || o.loop.option('vars').index;
				cache1 = LIT('[]');
			} else {
				// kvar = '_$'
				if (o.loop) {
			...
			if (o.treeType == 5) {
				cache1 = CALL(OP('.',scope.tagContextPath(),'$set'),[ctx,"'" + o.treeRef + "'"]);
				setter1 = OP('||',path,cache1);
			} else {
				setter1 = OP('=',path,OP('||',path,cache1));
			};
...
TagTree.prototype.altCache = function (){
	return this._owner.explicitKey() || this._owner.option('loop');
};
...
TagFragmentTree.prototype.altCache = function (){
	return true;
};
...
Scope.prototype.tagContextCache = function (){
	return this._tagContextCache || (this._tagContextCache = new TagCache(this,this.closure().declare("$",OP('.',this.tagContext(),'$'))));
};
...
function ModuleScope(){ return ClassScope.apply(this,arguments) };

subclass$(ModuleScope,ClassScope);
