...
Tag.prototype.consume = function (node){
	...
	if (node instanceof TagTree) {
		...
		console.log("CONSUME TAG TREE",this.parent() == o.par);
...
Tag.prototype.js = function (jso){
	...
	if (this.isSelf()) {
		...
		// if o:template
		return ("_T.$(" + typ + "," + o.body.c({expression: true}) + ").end()");
	} else {
		var ref = this.cacheRef();
		var typ1 = this.type().isClass() ? this.type().name() : ("'" + this.type()._value + "'");
		
		if (o.ivar) {
			o.path = OP('.',scope.context(),o.ivar).c();
			out = o.path + '||';
		} else if (ref) {
			out = ("" + (this.staticCache().c()) + "[" + (ref.c()) + "]||");
		};
		
		
		if (o.par && o.par.childRef()) {
			out += ("" + (this.staticCache().c()) + ".$(" + typ1 + "," + (ref.c()) + "," + (o.par.childRef().c()) + ")");
		} else if (ref && this.staticCache()) {
			out += ("" + (this.staticCache().c()) + ".$(" + typ1 + "," + (ref.c()) + ")");
		} else {
			if (o.ivar && o.path) {
				out += o.path + "=";
			};
