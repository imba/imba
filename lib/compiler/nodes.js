(function(){
	...
	ClassDeclaration.prototype.metadata = function (){
		...
		return {
			...
			desc: this._desc,
			superclass: (superclass_ = this.superclass()) && superclass_.namepath  &&  superclass_.namepath(),
			loc: this.loc()
		};
	};
	...
	MethodDeclaration.prototype.metadata = function (){
		...
		return {
			...
			desc: (_desc_ = this.scope()._desc) && _desc_.toDoc  &&  _desc_.toDoc(),
	...
	Scope.prototype.dump = function (){
		...
		desc.meta = self._node && self._node.metadata  &&  self._node.metadata();
		
		for (var i = 0, ary = iter$(self._annotations), len = ary.length, res = []; i < len; i++) {
			res.push(ary[i].metadata());
		};
		desc.entities = res;
	...
	ClassScope.prototype.namepath = function (){
		return this._node.name().c();
	};
	
	ClassScope.prototype.visit = function (){
		ClassScope.__super__.visit.apply(this,arguments);
		this._desc = this.stack().stash().pluck(Comment);
		return this;
	};
	...
	MethodScope.prototype.visit = function (){
		MethodScope.__super__.visit.apply(this,arguments);
		this._desc = this.stack().stash().pluck(Comment);
		return this;
	};
