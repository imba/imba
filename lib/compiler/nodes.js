...
ForOf.prototype.js = function (o){
	...
	if (this.options().own) {
		...
		return ("" + mark__(this.options().keyword) + "for (let " + (k.c()) + " in " + (o.c()) + ")") + code;
	};
};
...
Await.prototype.visit = function (o){
	...
	if (o.es6()) {
		if (fnscope) {
			...
			self.warn("toplevel await not allowed in es6");
		};
	};
...
ImportStatement.parse = function (str,startPos){
	...
	for (var i = 0, items = iter$(parts), len = items.length, part, ary; i < len; i++) {
		part = items[i];
