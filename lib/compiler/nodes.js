...
For.prototype.js = function (o){
	...
	if (this._tag && this._childTags) {
		...
		let cache = this._tag.fragment().cvar();
		let counter = this.tagvar('i');
...
Tag.prototype.js = function (o){
	...
	for (let i = 0, items = iter$(this._attributes), len = items.length, item; i < len; i++) {
		item = items[i];
		if (!isReactive) {
			if ((item instanceof TagFlag) && item.isStatic()) { continue; };
			add(("" + this.tvar() + "." + item.c(o)));
			// "{bvar} || ({tvar}.{item.c(o)})"
		} else if (item.isStatic()) {
			if (item instanceof TagFlag) { continue; };
			// what about whether our own tag is built or not?
			add(("" + (this.fragment().bvar()) + " || (" + this.tvar() + "." + item.c(o) + ")"));
		} else {
			let val = item.value();
			let iref = ("" + (this.fragment().cvar()) + "." + (item.oid()));
			item.setValue(LIT(("" + iref + "=" + this.vvar())));
			add(("" + this.vvar() + "=" + val.c(o)));
			
			if ((item instanceof TagFlagExpr) && optimizeFlag) {
				add(("" + this.vvar() + "===" + iref + " || " + this.ref() + ".flag$(" + item.value().c(o) + ")"));
			} else if (item instanceof TagFlag) {
				if (optimizeFlag) {
					add(("" + this.vvar() + "===" + iref + " || " + this.ref() + ".flag$(" + this.vvar() + " ? " + (item.quoted()) + " : ''," + item.value().c(o) + ")"));
				} else {
					add(("" + this.vvar() + "===" + iref + " || " + this.ref() + ".flagIf$(" + (item.quoted()) + "," + item.value().c(o) + ")"));
				};
			} else {
				add(("" + this.vvar() + "===" + iref + " || (" + this.ref() + "." + item.js(o) + ")"));
			};
		};
	};
	...
	for (let i = 0, len = nodes.length, item; i < len; i++) {
		...
		if (item instanceof Tag) {
			...
			if (isReactive) {
				add(("" + (this.fragment().bvar()) + " || " + this.tvar() + ".insert$(" + item.c(o) + "," + (slot++) + ")"));
			} else {
				add(("" + this.tvar() + ".insert$(" + item.c(o) + "," + (slot++) + ")"));
			};
		} else {
			// not if variable declaration etc
			// if this is the single child - things are different
			let id = item.oid();
			let cache = ("" + (this.fragment().cvar()));
			let key = ("" + (this.fragment().cvar()) + "." + id);
	...
	if (this._parent instanceof Loop) {
		// not for all?
		let i = this._parent.tagvar('i');
		if (this._parent.option('indexed')) {
			add(("" + i + "++"));
		} else {
			add(("" + (this._parent.tvar()) + ".push(" + this.tvar() + "," + i + "++," + this.kvar() + ")"));
		};
	};
