...
TagLoopFunc.prototype.visit = function (stack){
	...
	if (!this._isFast) {
		for (var i = 0, items = iter$(this._tags), len = items.length; i < len; i++) {
			items[i]._loopCache._callee = this.scope__().imbaRef('createTagMap');
		};
	};
	
	for (var j = 0, ary = iter$(this._params), len_ = ary.length; j < len_; j++) {
		ary[j].visit(stack);
	};
	
	if (len$(this._tags) == 1 && !this._tags[0].option('key') && this._isFast) {
