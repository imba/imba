...
}; exports.brace = __root.brace;

__root.normalizeIndentation = function (str){
	var m;
	var reg = /\n+([^\n\S]*)/g;
	var ind = null;
	...
	return str;
}; exports.normalizeIndentation = __root.normalizeIndentation;


__root.flatten = function (arr){
	var out = [];
	arr.forEach(function(v) { return (v instanceof Array) ? out.push.apply(out,__root.flatten(v)) : out.push(v); });
	return out;
}; exports.flatten = __root.flatten;


__root.pascalCase = function (str){
	return str.replace(/(^|[\-\_\s])(\w)/g,function(m,v,l) { return l.toUpperCase(); });
}; exports.pascalCase = __root.pascalCase;

__root.camelCase = function (str){
	str = String(str);
	// should add shortcut out
	return str.replace(/([\-\_\s])(\w)/g,function(m,v,l) { return l.toUpperCase(); });
}; exports.camelCase = __root.camelCase;

__root.dashToCamelCase = function (str){
	str = String(str);
	if (str.indexOf('-') >= 0) {
		// should add shortcut out
	...
	return str;
}; exports.dashToCamelCase = __root.dashToCamelCase;

__root.snakeCase = function (str){
	var str = str.replace(/([\-\s])(\w)/g,'_');
	return str.replace(/()([A-Z])/g,"_$1",function(m,v,l) { return l.toUpperCase(); });
}; exports.snakeCase = __root.snakeCase;

__root.setterSym = function (sym){
	return __root.dashToCamelCase(("set-" + sym));
}; exports.setterSym = __root.setterSym;

__root.quote = function (str){
	return '"' + str + '"';
}; exports.quote = __root.quote;

__root.singlequote = function (str){
	return "'" + str + "'";
}; exports.singlequote = __root.singlequote;

__root.symbolize = function (str){
	str = String(str);
	...
	return str;
}; exports.symbolize = __root.symbolize;


__root.indent = function (str){
	return String(str).replace(/^/g,"\t").replace(/\n/g,"\n\t").replace(/\n\t$/g,"\n");
}; exports.indent = __root.indent;

__root.bracketize = function (str,ind){
	if(ind === undefined) ind = true;
	if (ind) { str = "\n" + __root.indent(str) + "\n" };
	return '{' + str + '}';
}; exports.bracketize = __root.bracketize;

__root.parenthesize = function (str){
	return '(' + String(str) + ')';
}; exports.parenthesize = __root.parenthesize;

__root.unionOfLocations = function (){
	...
	return [a,b];
}; exports.unionOfLocations = __root.unionOfLocations;



__root.locationToLineColMap = function (code){
	...
	return map;
}; exports.locationToLineColMap = __root.locationToLineColMap;

__root.markLineColForTokens = function (tokens,code){
	return __root;
}; exports.markLineColForTokens = __root.markLineColForTokens;

__root.parseArgs = function (argv,o){
	var env_;
	if(o === undefined) o = {};
	...
	while ((i < argv.length)){
		...
		if (m = arg.match(/^\-([a-zA-Z]+)$/)) {
			...
			key = __root.dashToCamelCase(key);
	...
	return options;
}; exports.parseArgs = __root.parseArgs;

__root.printExcerpt = function (code,loc,pars){
	...
	var lines = code.split(/\n/g);
	var locmap = __root.locationToLineColMap(code);
	...
	var res = out.join('\n');
	return res;
}; exports.printExcerpt = __root.printExcerpt;

__root.printWarning = function (code,warn){
	var msg = warn.message; // b("{yellow('warn: ')}") + yellow(warn:message)
	var excerpt = __root.printExcerpt(code,warn.loc,{hl: 'whiteBright',type: 'warn',pad: 1});
	return msg + '\n' + excerpt;
}; exports.printWarning = __root.printWarning;
