...
# machine-readable docs for AI/LLM consumption - see scripts/build-llms.imba
app.get('/llms.txt') do(req,res)
	res.type('text/plain; charset=utf-8').send(llms.llms)

app.get('/llms-full.txt') do(req,res)
	res.type('text/plain; charset=utf-8').send(llms.full)

app.get('/sitemap.xml') do(req,res)
	res.type('application/xml').send(llms.sitemap)

app.get('/robots.txt') do(req,res)
	res.type('text/plain; charset=utf-8').send(llms.robots)

# every documented page is available as raw markdown at <href>.md
app.get(/^\/.+\.md$/) do(req,res)
	let page = llms.pages[req.path.slice(0,-3)]
	return res.sendStatus(404) unless page
	res.type('text/markdown; charset=utf-8').send(page.body)
...
app.get(/\.*/) do(req,res)
	let page = llms.pages[req.path]

	# serve markdown directly to clients that explicitly prefer it
	if page and req.accepts(['html','text/markdown']) == 'text/markdown'
		return res.type('text/markdown; charset=utf-8').send(page.body)

	# render html for generic (*/*) clients like curl and AI agents,
	# but keep returning 404 for requests explicitly wanting images
	let accept = req.headers.accept or '*/*'
	if accept.indexOf('image/') >= 0 and accept.indexOf('text/html') < 0
		return res.sendStatus(404)
	...
	res.send String <html>
		<head>
			...
			if page
				<link rel="alternate" type="text/markdown" href="{req.path}.md" title="{page.title} (markdown)">
