...
- [x] `docs/event-handling.md` — ends on a bare `## Listening to global events` heading with zero content. Write it (`imba.listen`/global event delegation) or cut the heading. *(Done 2026-08-14: written around `<global>` (listeners on `window`, auto-cleanup on unmount — verified in `dom/teleport.imba`), with live preview + `@keydown.esc`/`@click.outside` patterns. `imba.listen` is imba's emitter util, not DOM — deliberately not used here.)*
- [x] `docs/tags.md` (line 56) — placeholder `# add example for el.flags here` inside an empty code block. Write the example. *(Done 2026-08-14: add/remove/toggle/has example plus the stack-counted `incr`/`decr` API with auto-expiring `incr(name, ms)` — verified against `dom/flags.imba`.)*
- [x] `docs/data-binding.md` — `## Form Input Bindings` heading has no body before `## Examples`. Write intro prose or merge headings. *(Done 2026-08-14: wrote the two-way-binding intro under Form Input Bindings and removed the redundant `## Examples` heading.)*
- [x] `docs/style-syntax.md` — `#### Interpolation` (~line 312) has no body, and there is a second `## Interpolation` (~line 379). Merge into one section. *(Done 2026-08-14: moved the general interpolation prose + preview up into the Inline Styles subsection, deleted the standalone `## Interpolation`.)*
- [x] `docs/style-syntax.md` (line 100) — resolve `## Modifier Syntax [wip]`. *(Done 2026-08-14: section content was actually complete — dropped the `[wip]` tag.)*
- [x] `docs/classes.md` (line 103) — `## Lazy Getters [wip]`; content is repeated at ~152–158 under Meta Properties. Dedupe and drop the `[wip]`. *(Done 2026-08-14: removed the `[wip]` section (Meta Properties covers it); also fixed the dead self-referential "See [Meta Properties]" link and the dated "Upcoming versions of JavaScript" phrasing.)*
- [x] `guides/transitions.md` — ends mid-thought ("See properties." with no link). *(Done 2026-08-14: now links to `/docs/css/properties`.)*

### Stale content

- [x] `docs/assets.md` — presets still show `target: ['node12.19.0']` and `chrome58/firefox57/safari11/edge16`; "explained in more detail before final 2.0 release"; prune the unfinished "Future plans / Glob imports / Importing wasm" tail (~lines 214–243). *(Done 2026-08-14: preset block replaced with the actual `defaultConfig` from `src/bundler/config.imba` (base/node/esm/web/iife/client, chrome88/edge79/safari15, node14.13); pruned the "before final 2.0" promise and the Future-plans tail — confirmed glob imports are still unimplemented in the bundler.)*
- [x] `docs/teleports.md` — titled Teleports but only documents `<global>`; `<teleport>` exists in `packages/imba/typings/imba.d.ts` and is never documented. Document `<teleport>` or retitle. *(Done 2026-08-14: added "The Teleport Tag" section — `to` accepts selector or element (resolved via closest/querySelector), listeners attach to the target, cleanup on unmount — verified in `dom/teleport.imba`; with live preview.)*
- [x] `docs/router.md` (~lines 87–89) — "for the time being" tip reads as a long-lived temporary note; confirm and rewrite. *(Done 2026-08-14: verified in `router/route.imba` that prefix-matching is permanent designed behavior; rewrote the tip and documented the previously-undocumented `$` exact-match suffix (`route='/home$'`).)*
- [x] `guides/full_stack_deployment.md` — verify the `imba-base-template` repo still exists and `dist/src/server.loader.js` is still the emitted path. *(Done 2026-08-14: repo exists, but its pm2 config uses `./dist/server.js` — and an empirical build confirmed both `server.imba` and `src/server.imba` entries emit flat `dist/server.js` (no `dist/src/`, no `.loader.js`). Guide example fixed accordingly.)*
