...
function parseArgs(argv) {
  let args = {
    file: path.join("profiling", "sample-logic-heavy.imba"),
    runs: 500,
    warmup: 100,
    top: 30,
    writeProfile: null,
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (key == "--file") {
      ...
      console.log(`Usage: node profiling/profile-compile-cpu.mjs [options]

Options:
  --file <path>            Imba file to compile (default: profiling/sample-logic-heavy.imba)
  --runs <n>               profiled compile runs (default: 500)
  --warmup <n>             warmup compile runs before profiling (default: 100)
...
function summarizeProfile(profile, top) {
  ...
  let nodesSelf = [...selfUs.entries()]
    .map((entry) => rowFor(entry, "self"))
    .filter((row) => row.location.includes("src/compiler/nodes.mjs"))
    .sort((a, b) => b.ms - a.ms)
    .slice(0, top);

  let nodesInclusive = [...inclusiveUs.entries()]
    .map((entry) => rowFor(entry, "inclusive"))
    .filter((row) => row.location.includes("src/compiler/nodes.mjs"))
    .sort((a, b) => b.ms - a.ms)
    .slice(0, top);
...
async function main() {
  ...
  console.log(`Wall time: ${elapsedMs.toFixed(3)} ms (${(elapsedMs / args.runs).toFixed(3)} ms/compile)`);
  console.log(`Profile samples: ${summary.sampleCount}; sampled CPU: ${summary.totalMs.toFixed(3)} ms`);
  console.log(`Output: ${Buffer.byteLength(lastCompilation.js || "")} JS bytes, ${Buffer.byteLength(lastCompilation.css || "")} CSS bytes; diagnostics: ${lastCompilation.diagnostics?.length || 0}`);
  ...
  printRows("Top nodes.mjs self-time locations", summary.nodesSelf);
  printRows("Top nodes.mjs inclusive locations", summary.nodesInclusive);
}
