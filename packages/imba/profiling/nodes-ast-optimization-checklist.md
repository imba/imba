...
- [x] Run a V8 optimization/deoptimization trace on the full Lets corpus. `Node.c()` optimizes quickly after the no-options fast path; the remaining signal is mostly `wrong map` / lazy deopts across `c`, `js`, and `visit`, so future work should look at AST node hidden-class stability rather than more blind local rewrites.
