...
- [x] Run a V8 optimization/deoptimization trace on the full Lets corpus. `Node.c()` optimizes quickly after the no-options fast path; trace-deopt and `--log-ic` both show megamorphic loads in generic `Node.c` (`_cache`, `shouldParenthesize`, `js`, `_indentation`, `_options`) and traversal/codegen sites. The base `Node.setup()` already initializes `_cache = null`; a cache-slot/constructor-shape retest on the current copy stayed negative: short run was noise (+0.3% mean, +2.1% median), while the longer 5-round run was slower (-1.6% mean, -3.7% median). A `Stack.scope()` active-scope cache targeted the top large-file `_scope` LoadIC and was repaired after a stale sibling-scope verifier failure, but the longer Lets paired run was still slower (-1.8% mean, -2.1% median). A local `tokenTypeAt` replacement in `addImplicitIndentation` targeted deopt noise around `Rewriter.tokenType`, but the longer corpus run rejected it (-3.1% mean, -0.5% median). Splitting generic rewriter scan wrappers to isolate callback targets also lost on the longer corpus run (-3.6% mean, -4.3% median). This remains a larger specialization/design problem rather than a one-slot hidden-class fix.
...
- [ ] Redesign scope symbol tables. Recursive parent lookup, repeated global registration, variable resolution, and temporary allocation could become flatter and more cacheable with immutable root globals plus per-scope overlays. A narrow `RootScope.registerGlobal` bootstrap fast path was rejected on 2026-05-30: the smoke run was flat/noisy (+0.8% mean, +0.02% median) and the longer full Lets paired run was slightly slower (-0.38% mean, -0.16% median), so the existing `Scope.register` path was kept.
