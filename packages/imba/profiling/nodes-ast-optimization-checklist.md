...
- [x] Investigate `Scope.register` / `Scope.lookup` map costs. Tested `Object.create(null)` var maps with direct property checks on 2026-05-30; sample output stayed stable, but the full Lets paired benchmark was negative/noisy, so the normal object plus `hasOwnProperty` path was kept.
...
- [x] Run a V8 optimization/deoptimization trace on the full Lets corpus. `Node.c()` optimizes quickly after the no-options fast path; trace-deopt and `--log-ic` both show megamorphic loads in generic `Node.c` (`_cache`, `shouldParenthesize`, `js`, `_indentation`, `_options`) and traversal/codegen sites. A base-constructor `_cache = null` retest was negative, so this remains a larger specialization/design problem rather than a one-slot hidden-class fix.
