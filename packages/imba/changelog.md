...
* Fix duplicate setup and rendering of tags returned by functions in loops, such as `<(item.node!)>`. Render-context keys `0`, `false` and `''` were treated as missing, causing returned tags to initialize and render twice. Nested first-child chains compounded the duplicate work exponentially and could block the browser or server; they now render once per visit.

* Fix two compiler bugs around calls on `super`. A bang call on a super member (`super.foo!`) crashed the compiler with `this._args.traverse is not a function`, because the call rewrite handed the `Super` node a plain array where it expects an `ArgList`; it now compiles to `super.foo()` like the paren form, and a bare `super!` calls the parent method with no arguments (where bare `super` forwards `...arguments`). Chaining onto a super call dropped the call itself: `super.foo().concat(5)` compiled to `super.foo.concat(5)`, invoking `concat` on the inherited method rather than on its result — the same for `super.foo!.concat(5)`, index access (`super.foo()[0]`) and a super call passed as an argument. The call is now emitted before the chain.

* Watch project trees natively on macOS and Windows: one recursive `fs.watch` per root (the project, plus the package that owns each linked input outside it: the directory above its outermost `node_modules`, else the nearest `package.json`) instead of one chokidar `fs.watch` per file. chokidar 4+ has no FSEvents backend, so per-file watching cost one kqueue descriptor per file on macOS; a large project passed `OPEN_MAX` (10240), after which every reload's `cluster.fork` failed with `spawn EBADF` and the dev server died. Linux keeps chokidar, and `IMBA_WATCHER=chokidar` forces it elsewhere. `imbaconfig.json` gains `watch.ignore`, a gitignore-style list of paths the watcher never reports (`"watch": {"ignore": ["logs", ".verify"]}`; a bare name matches at any depth, a path containing a slash is anchored to the project root); `.git`, `.imba-cache`, `.cache`, `.DS_Store` and the build outdir are always ignored. The watcher no longer prints `add <path>` for every new file.

* Flush queued commits while the document is hidden (dev builds). The scheduler ticks on `requestAnimationFrame`, which never fires in hidden tabs — so state changed in a background tab (live sync, automation-driven events, an async `mount` finishing after data load) left commits queued forever and `await imba.commit!` never resolved until the tab was fronted. When `imba.scheduler.flushHidden` is on (defaults to on in dev builds, off in production) a scheduled tick whose queue contains a commit now flushes through a `MessageChannel` — unlike timers, message ports are not throttled in background tabs — so hidden documents render within milliseconds. Ticks with no queued commit (raf listeners, animations) stay frozen as before; a commit landing within a frame of the last tick retries on a throttled timer, so a self-committing render loop cannot spin unbounded while hidden; and a `visibilitychange` hook flushes commits whose rAF callback was already parked when the tab hid. Production behavior is unchanged unless opted in with `imba.scheduler.flushHidden = yes`. The scheduler singleton is now also exposed as `imba.scheduler`, and a stale parked rAF callback (fired on refront after a hidden flush already ran its tick) is ignored instead of producing an extra tick.
...
* Allow `when` and `by` to be used as regular identifiers — variable and field names, method and parameter names, object shorthand and destructuring, imports, etc. They are now contextual keywords, reserved only where the syntax needs them: `when`/`by` as guard/step after `for`/`while`/`until` on the same line, and `when` as a case-marker inside `switch` blocks. One consequence: a variable named `when` or `by` cannot be referenced inside a loop-header line itself (`for x in [1, when]`), since that position still reads as the guard/step keyword.
...
* Allow indented blocks only containing comments.
