...
const imbaVersion = typeof IMBA_VERSION == 'string' ? IMBA_VERSION : '*'
const ownVersion = typeof PKG_VERSION == 'string' ? PKG_VERSION : '0.0.0'

const usage = """
	Usage: create-imba [name] [options]
...
def toValidRepoName name
	return unless typeof name == 'string'
	return name if name == '.'
...
def main name, opts
	...
	try
		throw 1 unless parseInt(process.version.slice(1).split('.',1)[0]) >= 20
	catch
	...
	try
		process.chdir(dest) unless projectName == '.'
